...
class CouchConnector extends Service  {
  ...
  async _connect(attempts) {
    try {
      ...
      client = await nano(uri)
      let dbExists = await dbExists(options.dbName)
      if (!dbExists) {
        await this._client.db.create(datumoptions.dbName)
      }
      this._db = await this._client.db.use(datumoptions.dbName)
      return true
    } catch (err) {
